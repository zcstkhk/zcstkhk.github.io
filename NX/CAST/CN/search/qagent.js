...
function arrayIndexOf(array, value, begin, strict) 
{
...
function doSearch(win, qagent) {
	...
	selectCollection(collections[selectedCollections[collectionCount]].id, qagent);
...
function selectCollection(collectionId, qagent)
{
	//alert("Selecting " + collectionId);
	
	if(collectionId == "all")
	{
		if(top.topicSetArray.length == 0)
			alert("Collection level search is not available.");
		for(var i in top.topicSetArray)
		{	
			//alert("Selecting for all: " + i);
			qagent.selectCollection(top.topicSetArray[i]);
		}
	}
	else
		qagent.selectCollection(collectionId);
}
...
function _getMaxHits(qform) {
	...
	{
		var cookieValue = top.getCookie("maxHits");
		if(cookieValue)
		{
			maxh = cookieValue;
		}
...
function _getResultLayout(qform) {
	...
	{
		var cookieValue = top.getCookie("resultLayout");
		if(cookieValue)
		{
			l = cookieValue;
		}
...
function _getShowHighlighted(qform) 
{
	...
	{
		var cookieValue = top.getCookie("highlightOn");
		if(cookieValue)
		{
			if(cookieValue == "true")
			{
...
function _printCollectionChoice(doc, qagent) 
{
	...
	for(var i = 0; i < collections.length; i++) 
	{
		...
		if(collection.id == "all" || collection.id == top.courseId)
		{
			doc.write('\n<option value="' + i + '"');
...
function _loadCollections(doc, qagent)
{
	...
	{
		//alert(qagent.getCollectionId(i));
	...
	if(top.courseId)
	{
		...
		var index = arrayIndexOf(collectionIds, top.courseId, 0, true);
	...
	{
		var index = arrayIndexOf(collectionIds, "all", 0, true);
		var all = collections[index];
		collections.splice(index,1);
...
function _printSearchForm(doc, qagent) {
	...
	if(top.searchCollection && top.searchCollection == true)
	{
		doc.writeln('<tr>');
	...
	doc.writeln('<nobr><input type="image" src="../icons/go.gif" alt="' + findText + '" onClick="document.searchForm.submit();" VALUE="' + findText + '">');
	
	if(top.allowCookies2(false))
	{
...
function saveOptionsForm(doc)
{
	...
	window.opener.top.setCookie("highlightOn", doc.optionsForm.showHighlighted.checked);
	window.opener.top.setCookie("resultLayout", doc.optionsForm.resultLayout.options[doc.optionsForm.resultLayout.selectedIndex].value);
	window.opener.top.setCookie("maxHits", doc.optionsForm.maxhits.options[doc.optionsForm.maxhits.selectedIndex].value);
}
...
function showDocument(loc, qform, qagent, hitId) {
	_collectQueryOptions(qform);
  	...
  	locTopicSet = loc.substr(0,loc.indexOf("/"));
    	// if the topic set is not in the path, set it to top.courseId, because we are already inside of this topic set
  	if(locTopicSet == "" && top.courseId)
  	{
  		locTopicSet = top.courseId;
  	}
...
function showDocumentHL() {
	...
	//if the file to open is part of the current topicSet, open in the left frame (named main)
	if (locTopicSet == top.courseId) {
		if(navigator.appName.indexOf("Netscape") != -1) {
			var win = window.open("../search/qagent_highlight.html", "main");
			win.focus();
		}
		else {
			win = window.open("", "main");
			printLastHighlightedPage(win.document);
		}
	}
	...
	//if the file to open is part of a different topicSet, open the start file for that topicSet in a new window (the name of the start file differs depending on the type of deliverable - robo, tdoc, selfPaced)
		...
		var myLaunchFile = top.launchFile;
